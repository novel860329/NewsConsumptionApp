...
import static labelingStudy.nctu.minuku.config.Constants.ACTIVITY_STREAM_GENERATOR;
import static labelingStudy.nctu.minuku.config.Constants.APPUSAGE_STREAM_GENERATOR;
import static labelingStudy.nctu.minuku.config.Constants.BATTERY_STREAM_GENERATOR;
import static labelingStudy.nctu.minuku.config.Constants.CONNECTIVITY_STREAM_GENERATOR;
import static labelingStudy.nctu.minuku.config.Constants.TRANSPORTATION_STREAM_GENERATOR;
    ...
    public void updateStreamGenerators() {
        for(StreamGenerator streamGenerator: mRegisteredStreamGenerators.values()) {
            ...
            if(counter % streamGenerator.getUpdateFrequency() == 0) {
                ...
                if(streamGenerator.getClass().toString().equals(ACTIVITY_STREAM_GENERATOR)
                        || streamGenerator.getClass().toString().equals(APPUSAGE_STREAM_GENERATOR)
                        || streamGenerator.getClass().toString().equals(BATTERY_STREAM_GENERATOR)
                        || streamGenerator.getClass().toString().equals(CONNECTIVITY_STREAM_GENERATOR)
                        || streamGenerator.getClass().toString().equals(TRANSPORTATION_STREAM_GENERATOR))
                {
                    Log.d(TAG, "In Filter");
                    try{
